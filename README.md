...
- **PBS (sync / vérif / GC)** : variables dédiées `{var:job_id}`, `{var:datastore}` (source), `{var:remote}`, `{var:remote_store}` (destination), `{var:removed}` (place libérée par un Garbage Collect). Note : une **sync PBS** n'envoie ni durée ni taille dans sa notif (contrairement aux backups PVE) → `{var:duree}` / `{var:taille}` restent vides pour une sync.
