...
> **Aperçu avec vraies valeurs Home Assistant** : l'aperçu résout en direct les placeholders HA (`{state:...}`, `{attr:...}`, `{unit:...}`, Jinja HA) avec les **valeurs réelles** récupérées depuis Home Assistant (endpoint `POST /api/notifications/resolve-preview`, sans rien envoyer sur Discord). Un voyant indique l'état (🟢 valeurs en direct / 🔴 HA injoignable) et un bouton **« 🔄 Valeurs HA »** rafraîchit. Les `{var:...}` (remplis au déclenchement) restent affichés tels quels. Le bouton **« + entité »** de la barre d'outils ouvre un sélecteur d'entités **groupé par domaine** (💡 Lumières, 🔌 Interrupteurs, 📊 Capteurs, 🌡️ Chauffage/clim…) avec des **puces de filtre** par domaine, une **recherche** par nom/`entity_id`, et l'**état/valeur en direct** affiché à droite de chaque entité (ex. `21.4 °C`, `on`) ; l'entité choisie est insérée sous forme `{state:...|--}` à l'endroit du curseur. Les domaines de **moins de 5 entités** sont automatiquement regroupés dans un fourre-tout **« 🗃️ Divers »** pour garder la barre lisible.
...
- **PBS (sync / vérif / GC)** : variables dédiées `{var:job_id}`, `{var:datastore}` (source), `{var:remote}`, `{var:remote_store}` (destination), `{var:removed}` (place libérée par un GC), `{var:pending}` (données en attente de libération). Un **GC** a une durée → `{var:duree}` marche aussi. Note : une **sync PBS** n'envoie ni durée ni taille dans sa notif (contrairement aux backups PVE) → `{var:duree}` / `{var:taille}` restent vides pour une sync.
...
- **Backup multi-VM → tableau récap** : quand un job sauvegarde plusieurs VMs/CT, la variable **`{var:resume}`** fournit un **tableau compact, une ligne courte par VM** (`✅ 108 · Cine · 33s · 1.021 GiB`), dans **une seule notification** (jamais de pavé de logs). La couleur passe en **rouge** si **au moins une** VM a échoué. Les variables `{var:vmid}`, `{var:nom}`, etc. restent dispo (elles prennent la 1ʳᵉ VM) ; pour tout lister, utilise `{var:resume}`.
- Robustesse : les embeds sont **bornés aux limites Discord** (description 4096, champ 1024, total 6000, 25 champs max) — un log très long ne fait plus échouer l'envoi (le texte est tronqué avec …).
